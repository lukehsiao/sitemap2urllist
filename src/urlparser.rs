...
use crate::{
    ...
    error::{SiteMapError, SitemapError},
};

pub(crate) trait UrlParser {
    /// Parse URLs from the XML.
    async fn parse_urls(&self, cache: &Arc<Cache>) -> Result<Vec<Url>, SitemapError>;
}

/// Extract all URLs from an XML string body.
fn extract_urls(body: String) -> Result<Vec<Url>, SiteMapError) {
            ...
            if let Some(ref cv) = cache_value {
                if let Some(retry) = cv.retry_after {
                    if cv.timestamp + retry > Timestamp::now() {
                        debug!(timestamp=%cv.timestamp, retry_after=%retry, "skipping request due to 429, using sitemap from cache");

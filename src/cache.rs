...
/// dir, full disk) only logs a warning; failing the run here would report an
/// error exit for a run whose fetches and output all succeeded.
...
mod tests {
    ...
    fn cache_matches_hashmap_model(tc: hegel::TestCase) {
        let urls = vec![
            ...
            Url::parse("https://c.example/sitemap").unwrap(),
            Url::parse("https://d.example/sitemap.xml").unwrap(),
        ];

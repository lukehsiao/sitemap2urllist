            ...
            match r.status() {
                    ...
                    // Update cache
                    {
                        ...
                        if let Some(mut cv) = cache_value {
                            if status == StatusCode::NOT_MODIFIED {
                                debug!(url=%url.as_str(), status=status.as_str(), "got 304, using feed from cache");
                                body.clone_from(&cv.body);
                            } else {
                                debug!(url=%url.as_str(), status=status.as_str(), "cache hit, using feed from body");
                                cv.etag = etag;
                            ...
                            debug!(url=%url.as_str(), status=status.as_str(), "using feed from body and adding to cache");
                            cache.insert(
                                url.clone(),
                                CacheValue {
                ...
                StatusCode::TOO_MANY_REQUESTS => {
                    ...
                    if let Some(mut cv) = cache_value {
                        ...
                        debug!(url=%url.as_str(), response=?r, "got 429, using feed from cache");
                        cv.timestamp = Timestamp::now();
